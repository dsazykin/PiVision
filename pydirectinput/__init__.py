...
@_genericPyDirectInputChecks
def scroll(clicks=1, x=None, y=None, wheel_delta=120, _pause=True):
    """
        ...
        Args:
          clicks (int): The number of "clicks" to scroll.
                        ...
                        Default is 1.
          x (int, optional): The x-coordinate to move to before scrolling.
          y (int, optional): The y-coordinate to move to before scrolling.
        """
    if not x is None or not y is None:
        moveTo(x, y)  # Move mouse to position if specified

    # mouseData is an integer value, where a positive value indicates
    # scrolling up, and a negative value indicates scrolling down.
    # WHEEL_DELTA (120) is the standard value for one "click".
    scroll_amount = int(clicks * wheel_delta)

    extra = ctypes.c_ulong(0)
    ii_ = Input_I()
    ii_.mi = MouseInput(0, 0, scroll_amount, MOUSEEVENTF_WHEEL, 0, ctypes.pointer(extra))
    x = Input(ctypes.c_ulong(0), ii_)
    SendInput(1, ctypes.pointer(x), ctypes.sizeof(x))

@_genericPyDirectInputChecks
def hscroll(clicks=1, x=None, y=None, wheel_delta=120, _pause=True):
    """
    ...
    # mouseData is an integer value, where a positive value indicates
    # scrolling right, and a negative value indicates scrolling left.
    scroll_amount = int(clicks * wheel_delta)

    extra = ctypes.c_ulong(0)
    ii_ = Input_I()
    ii_.mi = MouseInput(0, 0, scroll_amount, MOUSEEVENTF_HWHEEL, 0, ctypes.pointer(extra))
    x = Input(ctypes.c_ulong(0), ii_)
    SendInput(1, ctypes.pointer(x), ctypes.sizeof(x))
...
@_genericPyDirectInputChecks
def hotkey(*keys, interval=0.0, logScreenshot=None, _pause=True):
    """
    ...
    Args:
      keys (str): The series of keys to press, in order. This can also be a
        list of key strings to press.
      interval (float, optional): The number of seconds in between each
        press/release. 0.0 by default.
      logScreenshot (optional): Ignored, but included for compatibility.
      _pause (bool, optional): Pauses after the function completes.
        Default is True.

    Returns:
      None
    """
    interval = float(interval)  # Ensure it's a float

    # Allow the user to pass a list/tuple of keys, e.g. hotkey(['ctrl', 'c'])
    if len(keys) == 1 and isinstance(keys[0], (list, tuple)):
        keys = keys[0]

    # Press keys down in order
    for key in keys:
        if len(key) > 1:  # 'c' vs 'ctrl'
            key = key.lower()
        # We call the internal functions directly, with _pause=False
        # so the main pause only happens *after* the whole hotkey is done.
        keyDown(key, _pause=False)
        time.sleep(interval)
    ...
    for key in reversed(keys):
        if len(key) > 1:
            key = key.lower()
        keyUp(key, _pause=False)
        time.sleep(interval)


shortcut = hotkey  # Alias for compatibility with PyAutoGUI
